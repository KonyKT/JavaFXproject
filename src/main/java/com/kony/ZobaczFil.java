...
public class ZobaczFil implements Initializable {
    @FXML
    private Text witaj;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        String hql = "FROM Ocena_filmu ";
        Query query = session.createQuery(hql);
        java.util.List results = query.list();
        Iterator it = results.iterator();
        while(it.hasNext()) {
            Ocena_filmu klient = (Ocena_filmu) it.next();
            if (Controller.zalogowany == klient.getId_uzytkownik()) {
                idakt.add(klient.getId_filmy());
                ocenki.add(klient.getOcena_filmu());
            }
        ...
        it = results.iterator();
        while(idakt.size() > i) {
            Filmy fil = (Filmy) it.next();
            if (fil.getId_filmy() == idakt.get(i)) {
                listaOcen.getItems().add(" Ocena:  " + ocenki.get(i)  + " \t\t\t Tytuł: " + fil.getTytul()  );
                i++;
            }
